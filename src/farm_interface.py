...
class FarmInterface:
    ...
    def mouse_over_buttons(self, x, y): # Hack so the buttons work together
        ...
        while (i < len(self.buttons)) and button_pressed == False:
            if contains(self.buttons[i].rectangle, sf.Vector2(x, y)) == False:
                i+=1
            else:
                button_pressed = True
    ...
    def on_mouse_button_released(self, button, x, y):
        if button == sf.Mouse.LEFT:
            if contains(self.load_button.rectangle, sf.Vector2(x, y)) and self.current_farm != self.student.farm:
                packet = net.Packet()
                packet.write(const.packet_request_load_farm)
                packet.write(self.student.client_id)
                self.client.send(packet)
            if contains(self.save_button.rectangle, sf.Vector2(x, y)):
